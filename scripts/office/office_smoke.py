...
class HandleOffice():
    ...
    def enter_office(self):
        touch(Template(r"tpl1742187921544.png", record_pos=(0.079, -0.212),
                       resolution=(1260, 2720)))

    def test_collect(self):
        touch(wait(Template(r"tpl1742187981288.png", record_pos=(0.017, 0.675),
                            resolution=(1260, 2720))))
        self.auto_collect("gold", "food", "solider")
        touch((0.5, 0.15))  # 关闭征收弹框

    def auto_collect(self, *args):
        """
        自动点击征收，直到征收全部
    ...
    def test_office(self):
        sleep(1)
        touch(wait(Template(r"tpl1742189189211.png", record_pos=(-0.304, -0.04),
                            resolution=(1260, 2720))), times=2)
        options = [(0.45, 0.63), (0.45, 0.70)]
        while True:
            """随机点击选项一或选项二、每次点击后判断是否道具不足，处理所有政务后，关闭弹框"""
            option = random.choice(options)
            touch(option)
            if exists(Template(r"tpl1742189883221.png", record_pos=(0.009, -0.189),
                               resolution=(1260, 2720))):
                touch((0.5, 0.15))  # 关闭道具获取途径弹框
                break
    ...
    def close_office_to_home(self):
        touch(wait(Template(r"tpl1742189582944.png", record_pos=(-0.431, -0.86),
                            resolution=(1260, 2720))))
        # 判断一下，是否存在拍脸弹框？
...
# powershell运行脚本如下
# $env:LOG_DIR="log\HW_mate60_pro";airtest run office_smoke.py --device "android://127.0.0.1:5037/192.168.110.175:55555" --log "log\HW_mate60_pro";
